...
6.) The stop() method can terminate all threads because it deletes the references to each thread maintained by the
    program. This is effective because each Runnable object, in its run() method, checks to see if the thread it's
    running on still exists. The getIndex() method returns a -1 index if the thread doesn't exist, which makes the if
    statement in the run() method false.