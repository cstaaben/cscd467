...
public class DragDropFiles extends JFrame {
	...
	private DefaultListModel<DefaultMutableTreeNode> model = new DefaultListModel<>();
    private int count = 0;
    private JTree tree;
...
//    private TreePath namesPath;
    ...
    private static AmazonS3 client = AmazonS3ClientBuilder.standard().withRegion(Regions.US_WEST_2).build();
        ...
        for(Bucket b : client.listBuckets()) {
        	...
        	do {
        		for(S3ObjectSummary summary : list.getObjectSummaries()) {
        			parent.add(new DefaultMutableTreeNode(summary.getKey()));
        		}
...
//        namesPath = tree.getPathForRow(2);
        ...
        tree.addTreeSelectionListener(new TreeSelectionListener() {
            public void valueChanged(TreeSelectionEvent e) {
                ...
                if(downloadPath == null) {
                	deleteBucketBtn.setEnabled(false);
                	deleteFileBtn.setEnabled(false);
			 }
                else if(isBucket(downloadPath.getLastPathComponent().toString())) {
                	deleteFileBtn.setEnabled(false);
                	deleteBucketBtn.setEnabled(true);
			 }
			 else {
                	deleteBucketBtn.setEnabled(false);
            ...
            public boolean importData(TransferHandler.TransferSupport info) {            	
                ...
                try {
                    ...
                    for (File f : l) {
                    		...
                    		client.putObject(path.getLastPathComponent().toString(), destFile.getName(), destFile);
                    		destFile.delete();
        ...
        download.addActionListener(new ActionListener() { 
        	  public void actionPerformed(ActionEvent e) { 
        	    ...
        	    if(downloadPath != null) {
			    	...
			    	S3Object object = client.getObject(downloadPath.getParentPath().getLastPathComponent().toString(), downloadPath.getLastPathComponent().toString());
        	    		try {
					...
					label.setText("Downloaded **" + object.getKey() + "** successfully");
				}
				catch(Exception ex) {
        	    			ex.printStackTrace();
				}
		...
		addBucket = new JButton("Add Bucket");
		addBucket.addActionListener(event -> {
			String prefix = JOptionPane.showInputDialog(null, "Enter a bucket prefix: ");
			
			while(bucketPrefixExists(prefix)) {
				prefix = JOptionPane.showInputDialog(null, "Prefix exists. Enter a new prefix:");
			}
			String bucket = prefix + UUID.randomUUID();
			
			client.createBucket(bucket);
			label.setText("Bucket created");
			
			treeModel.insertNodeInto(new DefaultMutableTreeNode(bucket), (DefaultMutableTreeNode)treeModel.getRoot(), treeModel.getChildCount(treeModel.getRoot()));
		});
		
		deleteFileBtn = new JButton("Delete File");
		deleteFileBtn.addActionListener(event -> {
			...
			if(choice == JOptionPane.YES_OPTION) {
				if(downloadPath != null && !isBucket(downloadPath.getLastPathComponent().toString())) {
					client.deleteObject(downloadPath.getParentPath().getLastPathComponent().toString(), downloadPath.getLastPathComponent().toString());
					label.setText("Deleted " + downloadPath.getLastPathComponent().toString());
					
					treeModel.removeNodeFromParent((DefaultMutableTreeNode) downloadPath.getLastPathComponent());
				}
			}
		});
		deleteFileBtn.setEnabled(false);
		
		deleteBucketBtn = new JButton("Delete Bucket");
		deleteBucketBtn.addActionListener(event -> {
			int choice = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete the "
					+ downloadPath.getLastPathComponent().toString() + " bucket?");
			
			if(choice == JOptionPane.YES_OPTION) {
				if(downloadPath != null && isBucket(downloadPath.getLastPathComponent().toString())) {
					for(S3ObjectSummary summary : client.listObjects(downloadPath.getLastPathComponent().toString()).getObjectSummaries()) {
						client.deleteObject(summary.getBucketName(), summary.getKey());
					}
					
					client.deleteBucket(downloadPath.getLastPathComponent().toString());
					
					DefaultMutableTreeNode node = (DefaultMutableTreeNode)treeModel.getChild(treeModel.getRoot(), treeModel.getIndexOfChild(treeModel.getRoot(), downloadPath.getLastPathComponent()));
...
//				treeModel.removeNodeFromParent((DefaultMutableTreeNode)downloadPath.getLastPathComponent());
        ...
        wrap.add(download);
        wrap.add(addBucket);
        wrap.add(deleteFileBtn);
        wrap.add(deleteBucketBtn);
        p.add(Box.createHorizontalStrut(4));
        p.add(Box.createGlue());
        p.add(wrap);
        p.add(Box.createGlue());
    ...
    private static boolean isBucket(String file) {
	    for(Bucket b : client.listBuckets()) {
		    if(b.getName().equalsIgnoreCase(file)) {
		    	return true;
		    }
	    }
    ...
    private static boolean bucketPrefixExists(String prefix) {
	    for(Bucket b : client.listBuckets()) {
		    if(b.getName().contains(prefix)) {
		    		return true;
		    }
	    }
